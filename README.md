...
The app is live on **Posit Connect Cloud**, git-backed: Connect watches this repo's `main` branch and
**auto-republishes on every push** (the same model as the sibling NEON apps), so the monthly data
refresh redeploys just by pushing the rebuilt bundle. There are no shinyapps secrets and no
`rsconnect` deploy step. The old **shinyapps.io** hosting (which retires at the end of 2026) has been
retired, along with `scripts/deploy.R`. The Connect Cloud `manifest.json` is generated by
`scripts/write_manifest.R` and is intentionally lean (bundle-only, no `neonUtilities`); the build
fails loudly if the heavy live-fetch packages `neonUtilities` or `arrow` ever leak back in. The
GitHub-Pages landing page (a social card and cold-start pre-warm in front of the Connect app) and the
full migration history are in **[DEPLOY.md](DEPLOY.md)**.
