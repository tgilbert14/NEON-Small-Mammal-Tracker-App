        ...
        card(full_screen = TRUE,
          card_head("activity", "Captures per plot, over time",
            info_pop("Captures per plot",
              ...
              p("All panels share one ", tags$b("y-scale"), ", so a busy plot really does sit higher than a quiet one. Values are ", tags$b("raw captures"), " (not effort-adjusted)."),
        ...
        card(
          card_head("hourglass-split", "How long do they live? — longest confirmed alive",
            info_pop("Longest confirmed alive",
              ...
              p("It's a ", tags$b("floor, not a lifespan"), ", and it's bounded ", tags$b("two ways"), ": animals still alive or that left the grid aren't counted (one not seen again is ", tags$em("presumed gone — death or dispersal"), ", never assumed dead); and careers beyond ~1.5 yr are set aside as probable ", tags$b("reused ear tags"), ", so the figure can't climb past ~1.7 yr even if the animal lived longer — that ceiling is why the abundant species pin near the same value."),
