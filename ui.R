  ...
  div(id = "mainTabsWrap", class = "main-tabs-wrap",
    navset_card_tab(id = "tabs",
      ...
      nav_panel(
        ...
        card(full_screen = TRUE,
          card_head("rulers", "Body measurements by species",
            info_pop("Measurements by species",
              p("The field measurements NEON records, summarised per species: the ", tags$b("median"), " with the ", tags$b("[min–max] range"), " and the number of records (", tags$b("n"), ")."),
              ...
              p(tags$em("Median + range, not a \"mode\" — for a continuous measurement like weight the median and spread are the honest summary. Counts confirmed species-level IDs only.")))),
